        ...
        <Tabs value={tab} onValueChange={setTab} className="space-y-6">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={tab} variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={{duration:0.22}}>
              <TabsContent value="papers" forceMount className={tab!=='papers'?'hidden':''}><QuestionSearch examType={LEVEL} subjects={SUBJECTS} initialSubject={searchSubject} accentFrom="from-amber-700" accentTo="to-red-700"/></TabsContent>
              <TabsContent value="subjects" forceMount className={tab!=='subjects'?'hidden':''}>
                <Card className="border-none shadow-xl bg-white dark:bg-[hsl(222,24%,12%)]">
                  <CardHeader className="border-b border-gray-100 dark:border-[hsl(222,18%,18%)] bg-gradient-to-r from-amber-50 to-orange-50 dark:from-[hsl(222,28%,13%)] dark:to-[hsl(222,25%,14%)] pb-5">
                    <CardTitle className="flex items-center gap-2 text-xl"><Book className="w-6 h-6 text-amber-600"/>My AS & A-Level Subjects</CardTitle>
